...
import { Plus, Search, Eye, AlertTriangle, ClipboardCheck } from 'lucide-react';
...
export default function QCForm() {
  ...
  }, [qcIssueRecords, search]);

  const stats = useMemo(() => {
    const totalQty = qcIssueRecords.reduce((s, r) => s + r.quantity, 0);
    const uniqueItems = new Set(qcIssueRecords.map(r => r.itemId)).size;
    return { count: qcIssueRecords.length, totalQty, uniqueItems };
  }, [qcIssueRecords]);
  ...
  const handleSave = () => {
    ...
    const result = createStockOut({
      ...
      remarks: formData.remarks || 'QC Issue',
      createdBy: 'admin',
    });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">QC Form</h1>
            <p className="text-sm text-gray-500">Issue quality control materials</p>
          </div>
        </div>
        <button onClick={() => setShowModal(true)} className="btn-primary flex items-center gap-2">
          <Plus className="w-4 h-4" /> New QC Issue
        </button>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="stat-card">
          <p className="text-xs font-medium text-gray-500 uppercase">Total QC Issues</p>
        ...
        <div className="stat-card">
          ...
          <p className="text-2xl font-bold text-teal-600 mt-1">{stats.totalQty}</p>
        </div>
        <div className="stat-card">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(record => (
                <tr key={record.id} className="hover:bg-gray-50">
                  <td className="table-cell font-medium text-teal-600">{record.issueNumber}</td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No QC issue records found</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="New QC Material Issue" maxWidth="max-w-3xl">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="label-field">QC Inspector *</label>
          ...
          <div>
            ...
            <input type="text" value={formData.remarks} onChange={(e) => setFormData({ ...formData, remarks: e.target.value })} className="input-field" placeholder="e.g. QC test sample" />
          </div>
        </div>
      ...
      <Modal isOpen={!!showDetail} onClose={() => setShowDetail(null)} title="QC Issue Details">
        {detailRecord && (
          <div className="space-y-3">
            {Object.entries({
              'Issue Number': detailRecord.issueNumber,
              'Issue Date': format(new Date(detailRecord.issueDate), 'dd MMM yyyy'),
              'Inspector': `${detailRecord.employeeName} (${detailRecord.employeeId})`,
